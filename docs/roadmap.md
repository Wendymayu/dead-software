...
| 主题 | 目录 | 核心演示 | 重要性 | 状态 |
|------|------|----------|--------|------|
| 🔴 RAG | `ai-architectures/rag/` | 查询 → 检索外部知识 → 增强生成 | 核心 | ❌ 未完成 |
| 🔴 ReAct | `ai-architectures/react/` | 思考→行动→观察循环，Agent 核心推理架构 | 核心 | ❌ 未完成 |
| 🔴 Tool Use | `ai-architectures/tool-use/` | LLM 选择并调用外部工具/函数 | 核心 | ❌ 未完成 |
| 🟡 Prompt Chaining | `ai-architectures/prompt-chaining/` | 多提示词串联，每步输出作为下步输入 | 重要 | ❌ 未完成 |
| 🟡 Router | `ai-architectures/router/` | 分类意图 → 路由到专门处理器 | 重要 | ❌ 未完成 |
| 🔴 Planning | `ai-architectures/planning/` | Agent 分解目标为子任务、制定计划、逐步执行 | 核心 | ❌ 未完成 |
| 🟡 Reflection | `ai-architectures/reflection/` | Agent 生成后自我审视并迭代改进 | 重要 | ❌ 未完成 |
| 🔴 Multi-Agent | `ai-architectures/multi-agent/` | 多个专长 Agent 协作，分工协商 | 核心 | ❌ 未完成 |
| 🔴 Memory | `ai-architectures/memory/` | 短期对话记忆 + 长期知识存储 + 工作记忆 | 核心 | ❌ 未完成 |
| 🔴 Guardrails | `ai-architectures/guardrails/` | 输入/输出验证层，约束 LLM 行为边界 | 核心 | ❌ 未完成 |
| 🔴 Human-in-the-Loop | `ai-architectures/human-in-the-loop/` | 关键决策点等待人类审批 | 核心 | ❌ 未完成 |
| 🔴 MCP | `ai-architectures/mcp/` | 标准化协议连接 LLM 与外部工具和数据源 | 核心 | ❌ 未完成 |
...
| 🔴 单例模式 | `design-patterns/singleton/` | 全局唯一实例（也必须了解其争议） | 核心 | ❌ 未完成 |
| 🔴 适配器模式 | `design-patterns/adapter/` | 不兼容接口之间的转换桥接 | 核心 | ❌ 未完成 |
| 🔴 命令模式 | `design-patterns/command/` | 将操作封装为对象，支持撤销和排队 | 核心 | ❌ 未完成 |
| 🔴 代理模式 | `design-patterns/proxy/` | 为对象提供替代品以控制访问 | 核心 | ❌ 未完成 |
| 🟡 外观模式 | `design-patterns/facade/` | 为复杂子系统提供简化接口 | 重要 | ❌ 未完成 |
| 🟡 组合模式 | `design-patterns/composite/` | 树形结构的统一处理 | 重要 | ❌ 未完成 |
| 🟢 桥接模式 | `design-patterns/bridge/` | 将抽象与实现分离 | 参考 | ❌ 未完成 |
| 🟡 模板方法 | `design-patterns/template-method/` | 父类定义骨架，子类填充步骤 | 重要 | ❌ 未完成 |
| 🟡 状态模式 | `design-patterns/state/` | 对象行为随内部状态变化 | 重要 | ❌ 未完成 |
| 🟢 迭代器模式 | `design-patterns/iterator/` | 顺序访问集合元素 | 参考 | ❌ 未完成 |
| 🟡 责任链模式 | `design-patterns/chain-of-responsibility/` | 请求沿处理链传递 | 重要 | ❌ 未完成 |
...
| 主题 | 目录 | 核心演示 | 重要性 | 状态 |
|------|------|----------|--------|------|
| 🔴 单一职责原则 | `solid/srp/` | 一个类/模块只做一件事 | 核心 | ❌ 未完成 |
| 🔴 开闭原则 | `solid/ocp/` | 扩展开放，修改关闭 | 核心 | ❌ 未完成 |
| 🟡 里氏替换原则 | `solid/lsp/` | 子类必须能替代父类 | 重要 | ❌ 未完成 |
| 🟡 接口隔离原则 | `solid/isp/` | 客户端不应依赖不需要的接口 | 重要 | ❌ 未完成 |
| 🔴 依赖反转原则 | `solid/dip/` | 高层模块不依赖低层模块 | 核心 | ❌ 未完成 |
...
| 主题 | 目录 | 核心演示 | 重要性 | 状态 |
|------|------|----------|--------|------|
| 🔴 生产者-消费者 | `concurrency/producer-consumer/` | 通过队列解耦生产和消费 | 核心 | ❌ 未完成 |
| 🟡 Reactor 模式 | `concurrency/reactor/` | 单线程事件循环处理多路 I/O | 重要 | ❌ 未完成 |
| 🟡 读写锁 | `concurrency/read-write-lock/` | 读共享、写互斥的并发控制 | 重要 | ❌ 未完成 |
| 🔴 Future/Promise | `concurrency/future-promise/` | 异步操作的占位对象 | 核心 | ❌ 未完成 |
...
| 主题 | 目录 | 核心演示 | 重要性 | 状态 |
|------|------|----------|--------|------|
| 🔴 God Object | `anti-patterns/god-object/` | 一个对象承担所有职责 | 核心 | ❌ 未完成 |
| 🔴 紧耦合 | `anti-patterns/tight-coupling/` | 模块间直接依赖具体实现 | 核心 | ❌ 未完成 |
| 🟡 过早优化 | `anti-patterns/premature-optimization/` | 在不确定瓶颈前就优化 | 重要 | ❌ 未完成 |
| 🔴 复制粘贴编程 | `anti-patterns/copy-paste-programming/` | 复制代码而非抽象共用逻辑 | 核心 | ❌ 未完成 |
